...
test('all five existing cases migrate; diagnostics do not require packages', () => {
  for (const id of ['aten-2029', 'autoptz-155', 'intelix', 'tcc2', 'zynthian-1530']) assert.ok(all().some(c => c.id === id))
...
test('selection covers case edits, shared changes, removals and documentation', () => {
  ...
  assert.equal(select(cases, ['lib/scripted-udp-device.mjs']).length, cases.length)
  assert.equal(select(cases, ['scripts/case.cjs']).length, cases.length)
  assert.equal(select(cases, null).length, cases.length)
